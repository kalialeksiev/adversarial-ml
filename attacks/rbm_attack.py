...
def rbm_attack(model_query, rbm_model, company_data, target_val,
    ...
    for _ in range(n_restarts):
        ...
        present = company_data.notna().to_numpy().reshape((-1, 1))
        ...
        for i in col_indices:
            if present[i]:
                ...
                x_test = np.copy(x)
                present_test = pd.copy(present)
                x_test[0][i] = None
                present_test[0][i] = False
